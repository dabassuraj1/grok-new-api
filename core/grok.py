...
class Grok:
    ...
    def __init__(self, model: str = "grok-3-auto", proxy: str = None) -> None:
        # Try to initialize with supported browser impersonation
        # Try multiple browser versions in order of preference (using available versions from curl_cffi)
        impersonate_versions = ["chrome124", "chrome123", "chrome120", "chrome119", "chrome116", "chrome110", "chrome107", "chrome104", "chrome101", "chrome100"]
        session_created = False

        for version in impersonate_versions:
            try:
                self.session: requests.session.Session = requests.Session(impersonate=version, default_headers=False)
        ...
        if not session_created:
            # Fallback to basic session if impersonation fails
            self.session: requests.session.Session = requests.Session()
            # Manually set headers to mimic Chrome
    ...
    def _load(self, extra_data: dict = None) -> None:
        ...
        if not extra_data:
            # Update session headers with LOAD headers
            self.session.headers.update(self.headers.LOAD)
    ...
    def c_request(self, next_action: str) -> None:
        ...
        # Clear session headers and set C_REQUEST headers
        self.session.headers.clear()
        self.session.headers.update(self.headers.C_REQUEST)
        self.session.headers.update({
            'baggage': self.baggage,
            'next-action': next_action,
            'sentry-trace': f'{self.sentry_trace}-{str(uuid4()).replace("-", "")[:16]}-0',
        })
        # Reorder headers to match expected order
        ordered_headers = Headers.fix_order(self.session.headers, self.headers.C_REQUEST)
        self.session.headers.clear()
    ...
    def start_convo(self, message: str, extra_data: dict = None) -> dict:
        ...
        # Clear session headers and set CONVERSATION headers
        self.session.headers.clear()
        self.session.headers.update(self.headers.CONVERSATION)
        self.session.headers.update({
            'baggage': self.baggage,
            'sentry-trace': f'{self.sentry_trace}-{str(uuid4()).replace("-", "")[:16]}-0',
            'x-statsig-id': xsid,
        ...
        # Reorder headers to match expected order
        ordered_headers = Headers.fix_order(self.session.headers, self.headers.CONVERSATION)
        self.session.headers.clear()
        self.session.headers.update(ordered_headers)
